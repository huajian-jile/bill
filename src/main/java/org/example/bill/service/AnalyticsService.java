...
public class AnalyticsService {
    ...
    public List<MonthDailyRowDto> month(
        ...
        AnalyticsChannel channel = AnalyticsChannel.fromParam(channelRaw);
        String ch =
                channel == AnalyticsChannel.wechat ? "WECHAT"
                        : channel == AnalyticsChannel.alipay ? "ALIPAY"
                        : null;
        return monthFromTransactions(year, month, loadTxsByChannel(userIds, ch));
    }

    private List<MonthDailyRowDto> monthFromTransactions(
            int year, int month, List<WechatBillTransaction> txs) {
        ...
        return buildMonthDailyRowsFromDayMaps(sums, txnCounts);
    }

    private static List<MonthDailyRowDto> buildMonthDailyRowsFromDayMaps(
            Map<LocalDate, BigDecimal[]> sums, Map<LocalDate, int[]> txnCounts) {
    ...
    /**
     * 全时段汇总 + 指定区间内各自然年分桶。一次拉取、一次扫描，行为与多次 {@link #byType} 一致，避免 N×3 次 HTTP。
     */
    public YearsRollupDto yearsRollup(
            int fromYear, int toYear, List<Long> userIds, String channelRaw) {
        if (fromYear > toYear) {
            throw new IllegalArgumentException("fromYear 不能大于 toYear");
        }
        AnalyticsChannel channel = AnalyticsChannel.fromParam(channelRaw);
        String ch =
                channel == AnalyticsChannel.wechat ? "WECHAT"
        ...
        IeAcc overall = new IeAcc();
        Map<Integer, IeAcc> byYear = new TreeMap<>();
        for (WechatBillTransaction t : loadTxsByChannel(userIds, ch)) {
            if (t.getAmountYuan() == null) continue;
            var od = TradeTimeUtil.parseDate(t.getTradeTime());
            if (od.isEmpty()) continue;
            ...
            overall.add(k, t.getAmountYuan());
            int y = d.getYear();
            if (y < fromYear || y > toYear) continue;
            byYear.computeIfAbsent(y, yk -> new IeAcc()).add(k, t.getAmountYuan());
        }
        AllTimeTotalsDto at =
                new AllTimeTotalsDto(
                        overall.income, overall.expense, overall.neutral,
                        overall.incomeC, overall.expenseC, overall.neutralC);
        List<YearBucketDto> rows = new ArrayList<>();
        for (Map.Entry<Integer, IeAcc> e : byYear.entrySet()) {
            IeAcc a = e.getValue();
            rows.add(
                    new YearBucketDto(
                            e.getKey(), a.income, a.expense, a.neutral,
                            a.incomeC, a.expenseC, a.neutralC));
        }
        return new YearsRollupDto(at, rows);
    }

    /**
     * 「某年」：该年收/支/中性汇总 + 12 个月每日行。单次加载、单次扫描，替代 3×by-type 与 12×month 请求。
     */
    public YearViewDto yearView(int year, List<Long> userIds, String channelRaw) {
        AnalyticsChannel channel = AnalyticsChannel.fromParam(channelRaw);
        String ch =
                channel == AnalyticsChannel.wechat ? "WECHAT"
                        : channel == AnalyticsChannel.alipay ? "ALIPAY"
                        : null;
        List<WechatBillTransaction> txs = loadTxsByChannel(userIds, ch);
        LocalDate yStart = LocalDate.of(year, 1, 1);
        LocalDate yEnd = LocalDate.of(year, 12, 31);
        IeAcc yearAcc = new IeAcc();
        List<Map<LocalDate, BigDecimal[]>> monthSums = new ArrayList<>(12);
        List<Map<LocalDate, int[]>> monthCnts = new ArrayList<>(12);
        for (int mo = 1; mo <= 12; mo++) {
            YearMonth ym = YearMonth.of(year, mo);
            TreeMap<LocalDate, BigDecimal[]> sums = new TreeMap<>();
            TreeMap<LocalDate, int[]> tc = new TreeMap<>();
        ...
        for (WechatBillTransaction t : txs) {
            ...
            if (od.isEmpty()) continue;
            LocalDate d = od.get();
            if (d.isBefore(yStart) || d.isAfter(yEnd)) continue;
            IncomeExpenseKind k = classifyIncomeExpense(t.getIncomeExpense());
            if (k != IncomeExpenseKind.SKIP) {
                yearAcc.add(k, t.getAmountYuan());
            }
            int mi = d.getMonthValue() - 1;
            Map<LocalDate, BigDecimal[]> sums = monthSums.get(mi);
            Map<LocalDate, int[]> tcs = monthCnts.get(mi);
            BigDecimal[] arr = sums.get(d);
            int[] c = tcs.get(d);
            if (arr == null || c == null) continue;
            String ie = t.getIncomeExpense();
            if (ie != null && ie.contains("收入")) { arr[0] = arr[0].add(t.getAmountYuan()); c[0]++; }
            else if (ie != null && ie.contains("支出")) { arr[1] = arr[1].add(t.getAmountYuan()); c[1]++; }
            else if (ie != null && ie.contains("中性")) { arr[2] = arr[2].add(t.getAmountYuan()); c[2]++; }
        }
        List<List<MonthDailyRowDto>> months = new ArrayList<>(12);
        for (int mi = 0; mi < 12; mi++) {
            months.add(buildMonthDailyRowsFromDayMaps(monthSums.get(mi), monthCnts.get(mi)));
        }
        YearBucketDto totals =
                new YearBucketDto(
                        year,
                        yearAcc.income,
                        yearAcc.expense,
                        ...
                        yearAcc.expenseC,
                        yearAcc.neutralC);
        return new YearViewDto(year, totals, months);
    }

    /** 收/支/中性 三档累计。 */
    private static final class IeAcc {
        ...
        void add(IncomeExpenseKind k, BigDecimal amt) {
            switch (k) {
                case INCOME -> { income = income.add(amt); incomeC++; }
                case EXPENSE -> { expense = expense.add(amt); expenseC++; }
                case NEUTRAL -> { neutral = neutral.add(amt); neutralC++; }
                default -> {}
            }
        }
    }
    ...
    private static final int COUNTERPARTY_CHART_TOP = 40;
    private static final int COUNTERPARTY_PAGE_MAX = 500;

    /** 按交易对方聚合，支持分页与 {@link CounterpartyRangeSummaryDto} 侧栏、{@code chartTop} 柱状。 */
    public CounterpartyBoardDto counterpartyBoard(
        ...
        List<CounterpartyGroupSummaryDto> all = new ArrayList<>();
        for (Map.Entry<String, CounterpartyAgg> e : map.entrySet()) {
            ...
            all.add(new CounterpartyGroupSummaryDto(
        ...
        all.sort(Comparator.comparing(CounterpartyGroupSummaryDto::expenseTotal)
                .reversed()
                .thenComparing(CounterpartyGroupSummaryDto::counterparty));
        long totalGroupCount = all.size();
        long ic = 0, ec = 0, nc = 0;
        for (CounterpartyGroupSummaryDto g : all) {
            ic += g.incomeCount();
            ec += g.expenseCount();
            nc += g.neutralCount();
        }
        CounterpartyRangeSummaryDto summary =
                new CounterpartyRangeSummaryDto(
                        ic, ec, nc, grandI, grandE, grandN);
        List<CounterpartyGroupSummaryDto> chartTop =
                all.stream()
                        .filter(
                                g ->
                                        g.expenseTotal().signum() > 0
                                                || g.incomeTotal().signum() > 0)
                        .sorted(
                                Comparator.comparing(CounterpartyGroupSummaryDto::expenseTotal)
                                        .reversed()
                                        .thenComparing(
                                                Comparator.comparing(
                                                                CounterpartyGroupSummaryDto
                                                                        ::incomeTotal)
                                                        .reversed()))
                        .limit(COUNTERPARTY_CHART_TOP)
                        .toList();
        int ps = Math.min(COUNTERPARTY_PAGE_MAX, Math.max(1, pageSize));
        int p0 = Math.max(0, page);
        int fromIdx = p0 * ps;
        List<CounterpartyGroupSummaryDto> pageSlice;
        if (fromIdx >= all.size()) {
            pageSlice = List.of();
        } else {
            int toIdx = Math.min(fromIdx + ps, all.size());
            pageSlice = new ArrayList<>(all.subList(fromIdx, toIdx));
        }
        return new CounterpartyBoardDto(
                pageSlice, grandI, grandE, grandN, totalGroupCount, p0, ps, chartTop, summary);
    }
